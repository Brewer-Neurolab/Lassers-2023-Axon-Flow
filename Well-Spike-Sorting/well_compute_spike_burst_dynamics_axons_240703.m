function [spike_burst_dyn_table]=well_compute_spike_burst_dynamics_axons_240703(allregion_unit_matched)
...
SR_vec=[]; regi_vec=[]; fi_vec=[]; ff_vec=[]; BurstBounds_vec={}; SpikesInBurstsPercent=[];
rowi=1;
for fi=1:length(allregion_unit_matched)
    ...
    for regi = 1:4
        ...
        for ui =1:length(timestamps)
            ...
            [ISI, ibi_sec, IBSR, spnb, bd_msec, SR_val,BurstBounds] = compute_spike_burst_dynamics_200304...
                (timestamps{ui}');
            end
            ...
            BurstBounds_vec{rowi}=BurstBounds;
            SR_vec(rowi) = SR_val;
            fi_vec(rowi) = fi; regi_vec(rowi) = regi;
            chan_name_cell = [chan_name_cell, string(chan2comp(ui))];
            SpikesInBurstsPercent(rowi)=sum(spnb)/sum(timestamps{ui});
...
spike_burst_dyn_table = table(fi_vec',regi_vec',chan_name_cell', SR_vec', ISI_cell', IBI_cell', IBSR_cell', spnb_cell', bd_cell',BurstBounds_vec',SpikesInBurstsPercent',....
    'VariableNames',{'fi','regi','channel_name', 'SpikeRate','ISI','IBI','IntraBurstSpikeRate','SpikeperBurst','BurstDuration','BurstBounds','SpikesInBurstsPercent'});
