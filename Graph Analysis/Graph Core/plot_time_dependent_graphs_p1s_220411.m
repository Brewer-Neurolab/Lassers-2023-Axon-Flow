...
%no stim
% source = load('D:\Brewer lab data\HFS\Temporal Analysis\No Stim\AFR Output\concatenated_source_table.mat');
% target = load('D:\Brewer lab data\HFS\Temporal Analysis\No Stim\AFR Output\concatenated_target_table.mat');
...
%% Plot r-squared histogram no thresh
% discontinued
for fi=2
    for graph_type=["source", "target"]
        for direction=["ff","fb"]
...
%             bins=convert_edges_2_centers(bins);
            ...
            title(graph_type+" rsq dist "+direction )
            ylim([0,0.5])
            xlabel("R-squared")
            ylabel("Probability")
            
            saveas(gcf,output_folder+graph_type+"_"+direction+"_"+"rsq_dist_nothresh","png")
        end
    end
end
...
%% Plot r-squared histogram with thresh concatonated
...
%% Plot slope histogram with thresh concatonated
...
%             saveas(gcf,output_folder+graph_type+"_"+direction+"_"+regList(regi)+"_"+"rsq_dist_thresh","png")
    ...
    title(t,"Slope Dist. "+regList(regi))
