%% Update 4/25/22: updated to have sliding time step
% added non uniform time scale
clear
clc
% outfolder = "D:\Brewer lab data\HFS\Temporal Analysis\40 HFS\AFR Output";
load("D:\Brewer lab data\HFS\Temporal Analysis\40 HFS\dataInfo.mat")
outfolder = "D:\Brewer lab data\HFS\Temporal Analysis\40 HFS\AFR Output Full Index nonuniform slide win 4pts";
% outfolder = "D:\Brewer lab data\HFS\Temporal Analysis\40 HFS\AFR Output 1bin 300s LM";
names=dataInfo.meaName;
% MFR_threshold = 0.0333;
MFR_threshold = 0.2;
bin_AFR_s = 0.1;
bin_lm_s=1; %functions as min time frame
no_points_thresh = 4; %differs from uniform calculation scripts bc functions as num points needed for lm
max_tf_s=15;
...
%     end
    ...
    for tunnel_type = ["FF","FB"]
        for well_type = ["source","target"]
            AFR_table = compute_allregion_AFR_table...
                (tunnel_type, well_type, MFR_threshold, bin_AFR_s, bin_lm_s, no_points_thresh, max_tf_s);
...
%% Funtions involved 
function AFR_table_w_lmfit = compute_allregion_AFR_table...
    (tunnel_type, well_type, MFR_threshold, bin_AFR_s, bin_lm_s, no_points_thresh, max_tf_s)
...
load('D:\Brewer lab data\HFS\Temporal Analysis\40 HFS\tunnelChannelList.mat')                                  %List of tunnel channels whether near target or source
load('D:\Brewer lab data\HFS\Temporal Analysis\40 HFS\dataInfo.mat')                                           %Names of MEA's and their orientations
% load("D:\Brewer lab data\HFS\Theta Stim\03-Mar-2022_B\allregion_unit_matched_stim.mat")                 %allregion structure with showing matching of sorted unit between tunnel channel sets and their directions
...
data_folder_addr = "D:\Brewer lab data\HFS\HFS Stim\full_index_pseudo_times"; %fixed from theta stim 4/7/22
...
AFR_table_w_lmfit = compute_allregion_time_dependent_linear_model...
    (AFR_table, bin_AFR_s,bin_lm_s, no_points_thresh, well_type, max_tf_s);
clear AFR_table;
...
function time_depn_AFR_table = compute_allregion_time_dependent_linear_model...
    (AFR_table, bin_AFR_s,bin_lm_s, no_points_thresh, well_type, max_tf_s)
...
time_depn_AFR_table.bin_width_s = cell(height(time_depn_AFR_table),1);
time_depn_AFR_table.start_times = cell(height(time_depn_AFR_table),1);
time_depn_AFR_table.num_pts = cell(height(time_depn_AFR_table),1);

% for t_start = 0.5*time_diff:bin_lm_s:total_time_s
% for t_start = 275:bin_AFR_s:total_time_s %testing

for rowi = 1:height(AFR_table)
    t_start=0.5*time_diff;
    tic
    while t_start<time_vec(end)
        
        %collect minimum num points
        num_points=0;
        end_pt=t_start+bin_lm_s-time_diff;
        idx = time_vec >= t_start & time_vec <=end_pt;
        
        while num_points<no_points_thresh
            % removing zeros
            ...
            num_points=length(tunnel_AFR);
%             time_span=time_vec(round(time_vec,10)==round(end_pt,10));
            bin_width_s=end_pt-t_start;
            if num_points<no_points_thresh 
                end_pt=end_pt+bin_AFR_s;
                if end_pt>time_vec(end) || bin_width_s>=max_tf_s
                    break
                end
                idx=time_vec >= t_start & time_vec <=end_pt;
            end
        ...
        if num_points<2 %&& ~(bin_width_s>=max_tf_s)
            t_start=end_pt;
            continue
        end
        
        time_span=time_vec(idx);
        bin_width_s=time_span(end)-time_span(1);
%         bin_width_s=end_pt-t_start;
        active_bins=time_span(~z_idx);
        next_AFR=active_bins(2);
        ...
        if length(tunnel_AFR) > no_points_thresh
            ...
            time_depn_AFR_table.bin_width_s{rowi} = [time_depn_AFR_table.bin_width_s{rowi},bin_width_s];
            time_depn_AFR_table.start_times{rowi} = [time_depn_AFR_table.start_times{rowi},active_bins(1)];
            time_depn_AFR_table.num_pts{rowi} = [time_depn_AFR_table.num_pts{rowi},num_points];
...
%             time_depn_AFR_table.bin_width_s{rowi} = [time_depn_AFR_table.bin_width_s{rowi},NaN];
%             time_depn_AFR_table.start_times{rowi} = [time_depn_AFR_table.start_times{rowi},NaN];
        end
            t_start=next_AFR;
    end
    disp("Row "+string(rowi)+" out of "+string(height(AFR_table))+" complete.")
    toc
end
end

% function time_depn_AFR_table = compute_allregion_time_dependent_linear_model(AFR_table, bin_AFR_s,bin_lm_s, no_points_thresh, well_type)
% % Computes time dependent linear mdl for AFR_table with time bins = time_s
% time_vec = AFR_table.Properties.CustomProperties.time_vec;
% time_diff = mean(unique(diff(time_vec)));
% total_time_s = time_vec(end);
...
% time_depn_AFR_table.start_times = cell(height(time_depn_AFR_table),1);
% 
% % for t_start = 0.5*time_diff:bin_AFR_s:total_time_s
% for t_start = 275:bin_AFR_s:total_time_s %testing
%     tic
%     for rowi = 1:height(AFR_table)
%         
...
%         end
%         
%         if length(tunnel_AFR) < no_points_thresh || isempty(idx)
%             continue
%         end
%         
%         time_span=time_vec(idx);
...
%             time_depn_AFR_table.p_val{rowi} = [time_depn_AFR_table.p_val{rowi}, pval];
%             time_depn_AFR_table.bin_width_s{rowi} = [time_depn_AFR_table.bin_width_s{rowi},bin_width_s];
%             time_depn_AFR_table.start_times{rowi} = [time_depn_AFR_table.start_times{rowi},t_start];
%             
% %         else
% %             
% %             time_depn_AFR_table.slope{rowi} = [time_depn_AFR_table.slope{rowi}, NaN];
...
% %         end
%     end
%     disp("Window Start at t = "+t_start+" completed for all channels.")
%     toc
% end
% end
