...
%% Set up save dir
parent_dir="D:\Brewer lab data\HFS\Graphic Summary Photos";
analysis_type="0.3 Threshold";
mydir=fullfile(parent_dir,string(my_id),analysis_type);
...
[nostim_slope_G,nostim_rsq_G]=cat_edges(nostim_allregion_unit_matched,nostim_source,nostim_target,NodeTable,nostim_datatable,my_id,rsq_thresh,"");
disp("No stim computed.")
exportgraphics(gcf,fullfile(mydir,"nostim_digraph")+".png")
% [hfs5_slope_G,hfs5_rsq_G]=cat_edges(hfs5_allregion_unit_matched,hfs5_source,hfs5_target,NodeTable,hfs5_datatable,my_id, rsq_thresh,"exponential");
% disp("5 HFS computed.")
[hfs40_slope_G,hfs40_rsq_G]=cat_edges(hfs5_allregion_unit_matched,hfs40_source,hfs40_target,NodeTable,hfs40_datatable,my_id, rsq_thresh,"");
disp("40 HFS computed.")
exportgraphics(gcf,fullfile(mydir,"hfs40_digraph")+".png")
% 
...
%% Bar Graph
figure
h1=histogram(nostim_rsq_G{my_id}.Edges.Weight,10);
exportgraphics(gcf,fullfile(mydir,"nostim_hist")+".png")
figure
h2=histogram(hfs40_rsq_G{my_id}.Edges.Weight,10);
exportgraphics(gcf,fullfile(mydir,"hfs40_hist")+".png")

figure

figure
nostim_rsq_weights=nostim_rsq_G{my_id}.Edges.Weight;
hfs40_rsq_weights=hfs40_rsq_G{my_id}.Edges.Weight;

weights=[nostim_rsq_weights;hfs40_rsq_weights];
labels=[repmat("nostim",[length(nostim_rsq_weights),1]);repmat("HFS40",[length(hfs40_rsq_weights),1])];

[p,tbl,stats]=anovan(weights,{labels});
[c,m]=multcompare(stats);

n_ff_nostim=nostim_rsq_G{my_id}.Edges.Weight(nostim_rsq_G{my_id}.Edges.Code==0.1);
n_fb_nostim=nostim_rsq_G{my_id}.Edges.Weight(nostim_rsq_G{my_id}.Edges.Code==0.6);

n_ff_hfs40=hfs40_rsq_G{my_id}.Edges.Weight(hfs40_rsq_G{my_id}.Edges.Code==0.1);
n_fb_hfs40=hfs40_rsq_G{my_id}.Edges.Weight(hfs40_rsq_G{my_id}.Edges.Code==0.6);

nostim_ff_wt=mean(nostim_rsq_G{my_id}.Edges.Weight(nostim_rsq_G{my_id}.Edges.Code==0.1));
nostim_fb_wt=mean(nostim_rsq_G{my_id}.Edges.Weight(nostim_rsq_G{my_id}.Edges.Code==0.6));

hfs40_ff_wt=mean(hfs40_rsq_G{my_id}.Edges.Weight(hfs40_rsq_G{my_id}.Edges.Code==0.1));
hfs40_fb_wt=mean(hfs40_rsq_G{my_id}.Edges.Weight(hfs40_rsq_G{my_id}.Edges.Code==0.6));
%% Related functions
...
function [G_cat_slopes,G_cat_rsq]=cat_edges(allregion_unit_matched,source,target,NodeTable,dataInfo,my_id,rsq_thresh,scaling)
...
for graph_type = ["rsq"] %to show increased reliability, was ["slope","rsq"]
    for fi=my_id
        ...
        for ti = 1:no_ti
            ...
            % Compute time subset
            source_subset = time_subset(source_table_full, ti, 0.1, 0.2, 0); %increased threshold to 0.6 from 0.2
            target_subset = time_subset(target_table_full, ti, 0.1, 0.2, 0); %increased threshold to 0.6 from 0.2
        ...
        if ~isempty(G_well_cat)
            ...
            %pick top % edge weights
            G_well_cat=rmedge(G_well_cat,find(G_well_cat.Edges.Weight<rsq_thresh));

            [gp,ax] = plot_graph(G_well_cat,dataInfo.orientation{fi}, graph_type, scaling);
            if strcmp(graph_type,"slope")
...
function [edge_obj,ax] = plot_graph(G, orientation, graph_type, scaling)
    if strcmpi(graph_type,'slope')
        max_lwidth = 2;
    else
        max_lwidth = 1;
    end
    
    LWidths = 10*G.Edges.Weight/max_lwidth;

    if scaling=="descrete" || scaling=="desc"
        LWidths(LWidths<=4.5)=4;
        LWidths(LWidths>4.5 & LWidths<=5.5)=8;
        LWidths(LWidths>5.5)=12;
    elseif scaling=="exponential" || scaling=="exp"
        LWidths=10.^(LWidths/5);
    end
...
%     arrow_obj.ShowArrows = 'off';
    ...
    % Drawing tunnel lines  
    rail_coords = [1,12; 5.5,5.5; 7.5,7.5];
    x_coord_id = [1,1,2,3]; y_coord_id = [3,2,1,1];
    for i=1:4
        line(rail_coords(x_coord_id(i),:),rail_coords(y_coord_id(i),:),...
            'color','k','lineWidth',2,'lineStyle','--')
    end
    
%     % Draw tunnel connect
%     x_coords = [[1:5, 8:12], [1:5, 8:12], 6.*ones(1,10), 6.*ones(1,10); ...
%         [1:5, 8:12], [1:5, 8:12], 7.*ones(1,10), 7.*ones(1,10)]';
%     y_coords = [ 6.*ones(1,10), 6.*ones(1,10), [1:5, 8:12], [1:5, 8:12]; ...
%         7.*ones(1,10), 7.*ones(1,10), [1:5, 8:12], [1:5, 8:12]]';
%     for i=1:length(x_coords)
%         line(x_coords(i,:), y_coords(i,:), 'color', 'k', 'linewidth',4)
%     end
    ...
    if strcmpi(orientation, 'cw')
        text([1,11,11,1],[12,12,1, 1],{'EC','DG','CA3','CA1'},...
            'FontWeight','bold','FontSize',20)
    else
        text([1,11,11,1],[12,12,1, 1],{'EC','CA1','CA3','DG'},...
            'FontWeight','bold','FontSize',20)
    end
    ax = gca;
    set(gca,'XTick',[],'YTick',[])
