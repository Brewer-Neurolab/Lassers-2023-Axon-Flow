...
% input_folder="D:\Brewer lab data\HFS\No Stim\full_index_pseudo_times";
% input_folder="D:\Brewer lab data\HFS\Theta Stim\full_index_pseudo_times";
input_folder="D:\Brewer lab data\HFS\HFS Stim\full_index_pseudo_times";
...
for i=1:length(recording_names)
    ...
    times_names_ff=times_names(contains(times_names,"times_") & contains(times_names,"_ff"));
    times_names_fb=times_names(contains(times_names,"times_") & contains(times_names,"_fb"));
    for j=1:length(times_names_ff)
        new_name=strsplit(times_names_ff(j),{'_'});
        times_names_ff(j)=new_name(2);
    end
    for j=1:length(times_names_fb)
        new_name=strsplit(times_names_fb(j),{'_'});
        times_names_fb(j)=new_name(2);
    end
    sorted_spike_data_ff=[];
    sorted_spike_data_fb=[];
    for j=1:length(channel_names)
        idx_to_load=find(strcmp(times_names_ff,channel_names{j}));
        if isempty(idx_to_load)
            sorted_spike_data_ff{j,1}=[];
            sorted_spike_data_fb{j,1}=[];
            continue
        end
        idx_to_load=idx_to_load(1);
        file_to_load="times_"+times_names_ff{idx_to_load}+"_ff.mat";
    ...
    for j=1:length(channel_names)
        idx_to_load=find(strcmp(times_names_fb,channel_names{j}));
        if isempty(idx_to_load)
            sorted_spike_data_fb{j,1}=[];
            sorted_spike_data_fb{j,1}=[];
            continue
        ...
        idx_to_load=idx_to_load(1);
        file_to_load="times_"+times_names_fb{idx_to_load}+"_fb.mat";
        load(strcat(current_folder,'\',file_to_load))
        num_clus=max(unique(cluster_class(:,1),'stable'));
        spikes=[];
        count=[];
