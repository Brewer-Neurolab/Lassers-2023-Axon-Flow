...
for tunnel_type = ["FF","FB"]
    for well_type = ["source","target"]
        AFR_table = compute_allregion_AFR_table(tunnel_type, well_type, MFR_threshold, bin_AFR_s, bin_lm_s, no_points_thresh);
...
%% Funtions involved 
function AFR_table_w_lmfit = compute_allregion_AFR_table(tunnel_type, well_type, MFR_threshold, bin_AFR_s, bin_lm_s, no_points_thresh)
...
load ../../mea_IO_basic/tunnelChannelList.mat                                  %List of tunnel channels whether near target or source
load ../../mea_IO_basic/dataInfo.mat                                           %Names of MEA's and their orientations
load ../../sorted-spike-analysis/allregion_unit_matching.mat                   %allregion structure with showing matching of sorted unit between tunnel channel sets and their directions

%% Initializing parameters
data_folder_addr = "../../data/";
...
for fi = 5
    tunnel_unit_mea = load(data_folder_addr+fi+dataInfo.meaName{fi}+"/sorted-burst/mea_tunnels");
    ...
    for regi = 1:4
        well_mea = load(data_folder_addr+fi+dataInfo.meaName{fi}+"/highpass_spikes/mea_"+well_regList(well_regi(regi)));
        ...
        for chani=1:length(tunnel_chan_idx_cell{regi})
            ...
            tunnel_channel_name = tunnel_unit_mea.mea.channel_names{tunnel_channel_idx};
                ...
                for ui=1:length(tunnel_unit_dir)
                    if tunnel_unit_dir(ui)
                        ...
                        tunnel_unit_spike_mat = [tunnel_unit_spike_mat, ...
                            tunnel_unit_mea.mea.sorted_spike_data{tunnel_channel_idx}{u_no}];
