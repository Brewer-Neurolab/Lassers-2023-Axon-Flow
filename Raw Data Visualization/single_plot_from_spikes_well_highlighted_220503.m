...
ax.FontSize=25;
%yline(threshold)
% title(strcat(only_folder,'-',last_electrode),'FontSize',30)
...
% target_bd=0.066; %CA3 no stim
% target_bd=0.043; %CA3 5hfs
% target_bd=0.041; %CA3 40hfs

% target_bd=0.086; %CA3 no stim
target_bd=0.057; %CA3 5hfs
% target_bd=0.055; %CA3 40hfs
...
%     xlim([t.("40hfs"){5}(i)-5,t.("40hfs"){5}(i)+5])
    ...
    xlims=round(xlims*sr,8);
    data_segment=data([xlims(1):xlims(2)]);
    ylim([min(data_segment),max(data_segment)])
    % only 10s windows
    %xlim([10*(i-1),10*i])
