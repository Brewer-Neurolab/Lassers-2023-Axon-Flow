function [combined_table]=combine_similar_times_nowobble_210719(table)
...
for i=1:rwtb
    ...
    for j=length(table.("Conduction Time"){i}):-1:2
        ...
        if (abs(table.("Conduction Time"){i}(j)-table.("Conduction Time"){i}(j-1))<0.00001)
            if j==2
                range_to_sum{range_ticker}=[subset_sum,1];
            else
                subset_sum=[subset_sum,j];
            end
        else
            if isempty(subset_sum)
                subset_sum=j;
